...
@pytest.fixture
def tls_ca_certificate_private_key_path(
    tls_certificate_authority: trustme.CA,
) -> Generator[str, None, None]:
    with tls_certificate_authority.private_key_pem.tempfile() as ca_private_key_path:
        yield ca_private_key_path
...
def gunicorn_process(
    ...
    with (
        ...
        with Process(args, stdout=output, stderr=output) as process:
            time.sleep(1)
            assert not process.poll()  # TODO: erroring here
